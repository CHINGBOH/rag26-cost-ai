...
def suggest_followup(question: str, answer: str, n: int = 3) -> str:
    ...
    try:
        ...
        system = (
            ...
            "要求：\n"
            "1. 追问必须严格基于回答中**明确引用了来源编号**的数据点或结论，不得基于推理、假设或常识生成问题\n"
            "2. 如果回答中包含【来源[X]】【来源[Y]】等引用标记，追问必须围绕这些被引用的具体信息展开\n"
            '3. 如果回答中没有带来源编号的具体数据，而是概括性说明（如\u201c根据现有资料\u201d），则不生成追问，返回空列表\n'
            '4. 禁止生成回答里没有明确内容的预测性或假设性问题（如\u201c如果X变化会怎样\u201d\u201c什么情况下会调整\u201d）\n'
            "5. 只输出 JSON，格式：{\"followups\": [{\"question\": \"...\", \"reason\": \"基于来源[X]的...\"}]}\n"
            "6. reason 字段必须说明该追问引用的是哪个来源编号的哪个具体信息点"
        )
        user = (
            f"原问题：{question}\n\n"
