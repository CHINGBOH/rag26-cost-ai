...
class UnifiedStore:
    ...
    def _init_keyword_store(self):
        ...
        try:
            self.keyword_client = Elasticsearch(
                self.config.keyword.hosts,
                maxsize=self.config.keyword.max_connections,
                timeout=self.config.keyword.timeout,
                # 基本认证
                basic_auth=(self.config.keyword.username, self.config.keyword.password)
    ...
    def _multi_recall(
        ...
        # 向量召回
        if self.vector_client and query.mode in ["vector", "hybrid"]:
            try:
        ...
        # 图谱召回
        if self.graph_driver and query.mode in ["graph", "hybrid"]:
            try:
                with self.graph_driver.session() as session:
                    # 简化查询：直接匹配Chunk内容
                    result = session.run(
    ...
    def _rerank(
        ...
        try:
            ...
            for item in candidates:
                ...
                # 添加文档标题信息（如果有）
                doc_name = getattr(item, "doc_name", None) or ""
            ...
            # Fallback: 使用简单的加权融合
            for item in candidates:
                item.rerank_score = item.keyword_score * 0.5 + item.vector_score * 0.5
            return sorted(candidates, key=lambda x: x.rerank_score, reverse=True)[
                : config.rerank_top_k
            ]
